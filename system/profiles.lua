...
mpi = di:taboption(this_tab, Value, "pincode", translate("PIN Code :")); 
mpi.optional=false; 
mpi.rmempty = true;
...
ml = di:taboption(this_tab, ListValue, "lock", translate("Block international roaming options & lock specific carriers :"));
ml:value("0", translate("No"))
ml:value("1", translate("Strong"))
ml:value("2", translate("Weak"))
ml.default=0
...
if nixio.fs.access("/etc/config/mwan3") then
	mlb = di:taboption(this_taba, ListValue, "lb", translate("Enable traffic load balancing 5G modules :"));
	mlb:value("0", translate("Disable"))
	mlb:value("1", translate("Enable"))
	mlb.default=0
end
...
alive:value("1", translate("Enable offline writing to system log"))
alive:value("2", translate("If the 5G module is enabled and goes offline, the route will automatically restart"))
alive:value("3", translate("Enable automatic redialing if the 5G module is disconnected"))
alive:value("4", translate("If the 5G module is enabled and disconnected, then power on the module again. Routing GPIO support is required, otherwise other methods will be used to reconnect"))
alive.default=0
...
cmpi = s:taboption(this_ctab, Value, "pincode", "PIN Code :"); 
cmpi.optional=false; 
cmpi.rmempty = true;
...
if nixio.fs.access("/etc/config/mwan3") then
	cmlb = s:taboption(this_ctaba, ListValue, "lb", translate("Enable traffic load balancing 5G modules :"));
...
calive.rmempty = true;
calive:value("0", translate("Disable"))
calive:value("1", translate("Enable offline writing to system log"))
calive:value("2", translate("Enabled with Router Reboot"))
calive:value("3", translate("Enabled with Modem Restart"))
calive:value("4", translate("If the 5G module is disconnected when enabled, then power on the module again. Routing GPIO support is required, otherwise other methods will be used to reconnect"))
calive.default=0
...
if (multilock == "0") or (multilock == "1" and rootlock == "1") then
	...
	bwday = s:taboption(this_ctab, ListValue, "bwday", translate("Day to Send Report"),
		translate("Day of month to send report for this Profile."))
	bwday.default = "0"
	bwday:value("0", translate("Disable"))
